...
func (s *simulator) nextNode(data *simData) *blockNode {
	...
	fmt.Printf("nextHeight %d, poolsize %v, immature %v, total %v, spendable %v, tickets bought %v @ %v\n",
		nextHeight, node.poolSize, len(s.immatureTickets), node.poolSize+uint32(len(s.immatureTickets)), 
                spendableSupply, len(ticketsAdded), dcrutil.Amount(ticketPrice))
